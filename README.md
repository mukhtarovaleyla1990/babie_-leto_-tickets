...
- **`config/festival.ts`** — название, дата, площадка, валюта, формат цены.
  Подставляются сразу везде: заголовок, футер, title вкладки, `/success`,
  `/cancel` и описание платежа в Stripe.
- **`config/content.ts`** — все остальные тексты интерфейса: хиро-блок
  (надпись сверху, абзац, кнопка, эмодзи-листья), заголовки шагов, плашки
  на карточках билетов и активностей, подписи полей формы, блок с итогом,
  страницы `/success` и `/cancel`, meta-теги и сообщения об ошибках.
  `{n}`, `{min}`, `{max}` — подстановки, их надо оставлять в строке:
...
config/
  festival.ts           название, дата, площадка, валюта
  content.ts            все тексты интерфейса
  tickets.ts            типы билетов и цены
  activities.ts         активности и места
